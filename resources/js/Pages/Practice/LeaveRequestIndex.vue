...
<template>
    <UserLayout>
        ...
        <div v-if="leaveRequests.data.length === 0" class="text-center py-16 bg-card rounded-2xl border anim-fade-in-up">
            ...
            <p class="text-sm text-muted-foreground mb-4">Ajukan izin pertamamu sekarang</p>
            <Link :href="route('leave-requests.create')">
                <Button size="sm">Ajukan Izin Sekarang</Button>
            </Link>
        </div>

        <div v-else class="space-y-3">
            <div v-for="item in leaveRequests.data" :key="item.id"
                 class="bg-card rounded-2xl border p-4 shadow-card hover:shadow-card-hover transition anim-fade-in-up">
