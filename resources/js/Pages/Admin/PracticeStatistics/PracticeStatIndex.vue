...
<template>
    <AdminLayout>
        ...
        <template #header-title>📊 Statistik Aktivitas Praktik</template>
        <template #header-sub>Monitoring aktivitas pengerjaan soal seluruh pengguna</template>

        <!-- Stats Overview -->
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-card border rounded-2xl p-5 hover:shadow-soft transition-shadow">
                <div class="flex items-center gap-3 mb-2">
                    <div class="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center text-lg">📝</div>
                    <span class="text-xs text-muted-foreground">Total Sesi</span>
                </div>
            ...
            <div class="bg-card border rounded-2xl p-5 hover:shadow-soft transition-shadow">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-xs text-muted-foreground">User Aktif</span>
                </div>
                <p class="text-2xl font-bold text-blue-600">{{ stats.totalUsers || 0 }}</p>
            </div>
            <div class="bg-card border rounded-2xl p-5 hover:shadow-soft transition-shadow">
                <div class="flex items-center gap-3 mb-2">
                    <div class="w-9 h-9 rounded-xl bg-fern/10 flex items-center justify-center text-lg">📈</div>
                    <span class="text-xs text-muted-foreground">Rata-rata Skor</span>
                </div>
                <p class="text-2xl font-bold text-fern">{{ stats.avgScore || '0.0' }}</p>
            </div>
            <div class="bg-card border rounded-2xl p-5 hover:shadow-soft transition-shadow">
                <div class="flex items-center gap-3 mb-2">
                    <div class="w-9 h-9 rounded-xl bg-emerald-100 flex items-center justify-center text-lg">🎯</div>
                    <span class="text-xs text-muted-foreground">Akurasi</span>
                </div>
                <p class="text-2xl font-bold text-emerald-600">{{ accuracyPct() }}%</p>
            </div>
        </div>

        <!-- Filters -->
        <div class="bg-card rounded-2xl border p-5 mb-6 shadow-sm">
            <h3 class="text-sm font-semibold mb-4 text-muted-foreground">🔍 Filter & Export</h3>
            <div class="flex flex-col sm:flex-row flex-wrap gap-3">
                <div class="flex flex-col gap-1">
                    <label class="text-xs text-muted-foreground">Tanggal Mulai</label>
                    <input v-model="start" type="date" class="px-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-ring w-full sm:w-40" />
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-xs text-muted-foreground">Tanggal Akhir</label>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <select v-model="status" class="px-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-ring w-full sm:w-36">
                ...
                <div class="flex items-end gap-2 flex-wrap">
                    <button @click="applyFilters" class="inline-flex items-center gap-1.5 px-4 py-2 bg-primary text-primary-foreground text-sm font-medium rounded-lg hover:bg-primary/90 transition">
                    ...
                    <button @click="resetFilters" class="inline-flex items-center gap-1.5 px-4 py-2 border border-border text-sm font-medium rounded-lg hover:bg-muted transition">
                        Reset
                    </button>
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden mb-6">
            <div class="px-5 py-4 border-b flex items-center justify-between">
                <h3 class="font-semibold text-sm">Data Sesi Pengerjaan</h3>
                <span class="text-xs text-muted-foreground bg-muted px-2 py-1 rounded-full">{{ sessions.total || 0 }} data</span>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b bg-muted/30">
                            ...
                            <th class="text-center px-4 py-3 text-xs font-semibold text-muted-foreground whitespace-nowrap">Status</th>
                            <th class="text-left px-4 py-3 text-xs font-semibold text-muted-foreground whitespace-nowrap">Tanggal</th>
                            <th class="text-center px-4 py-3 text-xs font-semibold text-muted-foreground whitespace-nowrap">Aksi</th>
                        </tr>
                    </thead>
                        ...
                        <tr v-if="sessions.data.length === 0">
                            <td colspan="9" class="px-4 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-muted-foreground text-sm">Tidak ada data yang ditemukan</p>
                                </div>
                            </td>
                        </tr>
                        <tr v-for="(s, idx) in sessions.data" :key="s.id" class="hover:bg-muted/30 transition-colors">
                            <td class="px-4 py-3.5 text-xs text-muted-foreground font-mono">{{ (sessions.from || 0) + idx }}</td>
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-medium text-xs truncate max-w-[120px]">{{ s.user?.name || '-' }}</p>
                                        <p class="text-[10px] text-muted-foreground truncate max-w-[120px]">{{ s.user?.email || '' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3.5">
                                <p class="text-xs font-medium truncate max-w-[150px]">{{ s.package?.title || '-' }}</p>
                                <p v-if="s.card_id" class="text-[10px] text-muted-foreground mt-0.5 truncate max-w-[150px]">Card: {{ s.card_id }}</p>
                            </td>
                            <td class="px-4 py-3.5 text-center text-xs font-medium">{{ s.total_question || 0 }}</td>
                            <td class="px-4 py-3.5 text-center">
                                <span class="text-xs font-semibold text-emerald-600">{{ s.correct_answer || 0 }}</span>
                                <span class="text-[10px] text-muted-foreground">/{{ s.total_question || 0 }}</span>
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                <span :class="['text-sm font-bold px-2 py-0.5 rounded-lg', scoreColor(s.total_score), scoreBg(s.total_score)]">
                            ...
                            <td class="px-4 py-3.5 text-xs text-muted-foreground whitespace-nowrap">{{ formatDate(s.created_at) }}</td>
                            <td class="px-4 py-3.5 text-center">
                                <Link :href="route('admin.practice-statistics.show', s.id)"
                                      class="inline-flex items-center gap-1 text-xs font-medium text-primary hover:text-primary/80 hover:underline transition">
                                    Detail →
                                </Link>
                            </td>
                        </tr>
