        ...
        <div>
            <form @submit.prevent="submit" enctype="multipart/form-data">
                    ...
                    <div class="p-5 sm:p-6">
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-muted-foreground mt-1 leading-relaxed">Unggah foto profil pengguna. Format JPG, PNG, atau WebP. Ukuran maksimal 2MB.</p>
                ...
                <div class="bg-card rounded-2xl border border-border/60 shadow-sm overflow-hidden mb-6 anim-fade-in-up anim-delay-1">
                    ...
                    <div class="p-5 sm:p-6 space-y-4">
                        <div class="space-y-2">
                            ...
                            <Input v-model="form.name" required placeholder="Masukkan nama lengkap" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                        ...
                        <div class="space-y-2">
                            ...
                            <Input v-model="form.email" type="email" required placeholder="email@contoh.com" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <Label class="text-sm font-semibold">Password <span class="text-destructive">*</span></Label>
                                <Input v-model="form.password" type="password" required minlength="8" placeholder="Minimal 8 karakter" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                                <p v-if="form.errors.password" class="text-xs text-destructive flex items-center gap-1.5 mt-1">
                                    <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
                                    {{ form.errors.password }}
                                </p>
                            </div>
                            <div class="space-y-2">
                                <Label class="text-sm font-semibold">Konfirmasi Password <span class="text-destructive">*</span></Label>
                                <Input v-model="form.password_confirmation" type="password" required placeholder="Ulangi password" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-5 sm:p-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <Input v-model="form.student_class" placeholder="Contoh: X IPA 1" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                            </div>
                            <div class="space-y-2">
                                <Label class="text-sm font-semibold">Bidang</Label>
                                <Input v-model="form.bidang" placeholder="Contoh: Matematika" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                            </div>
                            <div class="space-y-2">
                                <Label class="text-sm font-semibold">Level</Label>
                                <Input v-model="form.level" placeholder="Contoh: Pemula" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                            </div>
                            <div class="space-y-2">
                                <Label class="text-sm font-semibold">Sekolah</Label>
                                <Input v-model="form.school_name" placeholder="Nama sekolah" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-5 sm:p-6 space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <Input v-model="form.phone" placeholder="08xxx" class="h-11 rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                            </div>
                            <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <Textarea v-model="form.address" :rows="2" placeholder="Alamat lengkap" class="resize-none rounded-xl bg-muted/50 border-border/60 focus:bg-background transition-all duration-200" />
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-card rounded-2xl border border-border/60 shadow-sm p-5 sm:p-6 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 anim-fade-in-up anim-delay-4">
                    <Link :href="route('admin.users.index')" class="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl border bg-background text-sm font-semibold hover:bg-accent transition-all duration-200 active:scale-[0.98]">
