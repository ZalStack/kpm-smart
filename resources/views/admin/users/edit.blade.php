...
@section('content')
<div class="space-y-6">
    <a href="{{ route('admin.users.show', $user->id) }}" class="text-primary hover:text-brand-800 text-sm inline-flex items-center gap-2 transition-colors group">
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="admin-card p-5 md:p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="password" name="password" minlength="8" class="form-input" placeholder="Kosongkan jika tidak diubah">
                    @error('password') <p class="text-xs text-danger-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="form-label">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" minlength="8" class="form-input">
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="student_class" value="{{ old('student_class', $user->student_class) }}" class="form-input">
                </div>
                <div>
                    <label class="form-label">Bidang</label>
                    <input type="text" name="bidang" value="{{ old('bidang', $user->bidang) }}" class="form-input">
                </div>
                <div>
                    <label class="form-label">Level</label>
                    <input type="text" name="level" value="{{ old('level', $user->level) }}" class="form-input" placeholder="Contoh: Pemula, Menengah, Lanjut">
                </div>
                <div>
                    <label class="form-label">Sekolah</label>
                    <input type="text" name="school_name" value="{{ old('school_name', $user->school_name) }}" class="form-input">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-border">
            ...
            <a href="{{ route('admin.users.show', $user->id) }}" class="btn-secondary">Batal</a>
        </div>
    </form>
</div>
