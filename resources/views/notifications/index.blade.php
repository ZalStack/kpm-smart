...
@section('title', 'Notifikasi - KPM Belajar Online')
...
<style>
    ...
    .notif-stagger > *:nth-child(1) { animation-delay: 0ms; }
    .notif-stagger > *:nth-child(2) { animation-delay: 60ms; }
    .notif-stagger > *:nth-child(3) { animation-delay: 120ms; }
    .notif-stagger > *:nth-child(4) { animation-delay: 180ms; }
    .notif-stagger > *:nth-child(5) { animation-delay: 240ms; }
</style>

<div class="max-w-3xl mx-auto space-y-6 notif-stagger">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <span class="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold mb-2">🔔 Notifikasi</span>
            <h2 class="text-2xl md:text-3xl font-bold text-foreground font-display">Notifikasi</h2>
        ...
        <button onclick="markAllRead()" id="markAllBtn"
            class="px-4 py-2 rounded-lg text-sm font-semibold bg-gradient-to-r from-navy-light to-navy text-white hover:shadow-lg transition-all duration-200 shadow-md shadow-navy/15 active:scale-95">
            Tandai Semua Dibaca
        </button>
    </div>
    ...
    <div class="space-y-3" id="notifList">
        @forelse($notifications as $notif)
            <div class="notif-card group bg-card rounded-lg border {{ $notif->isRead() ? 'border-border' : 'border-primary/30 bg-primary/[0.02]' }} p-4 sm:p-5 transition-all duration-200 hover:shadow-lg hover:shadow-navy/5 hover:border-border cursor-pointer"
                 data-id="{{ $notif->id }}" data-read="{{ $notif->isRead() ? '1' : '0' }}">
                <div class="flex items-start gap-3 sm:gap-4">
                    {{-- Icon --}}
                    <div class="w-10 h-10 sm:w-11 sm:h-11 rounded-lg flex items-center justify-center flex-shrink-0
                        @if($notif->type === 'order') bg-gold-400/10
                        @elseif($notif->type === 'testimonial') bg-primary/10
        ...
        card.addEventListener('click', function() {
            if (this.dataset.read === '0') {
                ...
                fetch(`/notifications/${id}/read`, {
                    ...
                    this.dataset.read = '1';
                    this.classList.remove('border-primary/30', 'bg-primary/[0.02]');
...
function markAllRead() {
    ...
    fetch('{{ route("notifications.mark-all-read") }}', {
        ...
        document.querySelectorAll('.notif-card').forEach(card => {
            card.dataset.read = '1';
            card.classList.remove('border-primary/30', 'bg-primary/[0.02]');
